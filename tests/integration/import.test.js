...
const askPassphrase = () => Promise.resolve(undefined);
...
describe('import procedure', function () {
  ...
  it('can import Legacy', async () => {
    await WalletImport.processImportText('KztVRmc2EJJBHi599mCdXrxMTsNsGy3NUjc3Fb3FFDSMYyMDRjnv', askPassphrase);
  ...
  it('can import Legacy P2SH Segwit', async () => {
    await WalletImport.processImportText('L3NxFnYoBGjJ5PhxrxV6jorvjnc8cerYJx71vXU6ta8BXQxHVZya', askPassphrase);
  ...
  it('can import Legacy Bech32 Segwit', async () => {
    await WalletImport.processImportText('L1T6FfKpKHi8JE6eBKrsXkenw34d5FfFzJUZ6dLs2utxkSvsDfxZ', askPassphrase);
  ...
  it('can import HD Legacy Electrum (BIP32 P2PKH)', async () => {
    await WalletImport.processImportText('eight derive blast guide smoke piece coral burden lottery flower tomato flame', askPassphrase);
  ...
  it('can import HD Electrum (BIP32 P2WPKH)', async () => {
    await WalletImport.processImportText('noble mimic pipe merry knife screen enter dune crop bonus slice card', askPassphrase);
  ...
  it('can import AEZEED with password', async () => {
    await WalletImport.processImportText(
      ...
      () => Promise.resolve('strongPassword'),
    );
    assert.strictEqual(lastImportedWallet.type, HDAezeedWallet.type);
  });
  ...
  it('importing empty BIP39 should yield BIP84', async () => {
    ...
    await WalletImport.processImportText(tempWallet.getSecret(), askPassphrase);
    assert.strictEqual(lastImportedWallet.type, HDSegwitBech32Wallet.type);
  });

  it('can import Legacy with uncompressed pubkey', async () => {
    await WalletImport.processImportText('5KE6tf9vhYkzYSbgEL6M7xvkY69GMFHF3WxzYaCFMvwMxn3QgRS', askPassphrase);
  ...
  it('can import BIP38 encrypted backup', async () => {
    await WalletImport.processImportText('6PnU5voARjBBykwSddwCdcn6Eu9EcsK24Gs5zWxbJbPZYW7eiYQP8XgKbN', askPassphrase);
  ...
  it('can import watch-only address', async () => {
    await WalletImport.processImportText('1AhcdMCzby4VXgqrexuMfh7eiSprRFtN78', askPassphrase);
    assert.strictEqual(lastImportedWallet.type, WatchOnlyWallet.type);
    await WalletImport.processImportText('3EoqYYp7hQSHn5nHqRtWzkgqmK3caQ2SUu', askPassphrase);
    assert.strictEqual(lastImportedWallet.type, WatchOnlyWallet.type);
    await WalletImport.processImportText('bc1q8j4lk4qlhun0n7h5ahfslfldc8zhlxgynfpdj2', askPassphrase);
    assert.strictEqual(lastImportedWallet.type, WatchOnlyWallet.type);
    await WalletImport.processImportText(
